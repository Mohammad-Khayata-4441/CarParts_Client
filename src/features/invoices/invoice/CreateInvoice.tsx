...
export default (props: Props) => {
  ...
  const onSubmit = async (data: AddInvoiceDto) => {
    ...
    try {
      ...
      queryClient.invalidateQueries(`client-account-${data.clientId}`)
  ...
  useEffect(() => { setValue('invoiceType', props.invoiceType) }, [props.invoiceType])
    ...
    <Dialog open={props.is} maxWidth={"md"} fullWidth>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <DialogActions >
          ...
          <Button variant="contained" disabled={loading} type="submit" sx={{ minWidth: 150 }} endIcon={loading ? <CircularProgress sx={{ height: 20 }} size={18} color="inherit" /> : <Receipt />}>حفظ</Button>
        </DialogActions>
      </form>
    </Dialog>
  );
